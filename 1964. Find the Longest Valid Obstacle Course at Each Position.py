...
Return an array ans of length n, where ans[i] is the length of the longest obstacle course for index i as described
above.
...
def longestObstacleCourseAtEachPosition(obstacles):
    ...
    for i in range(len(obstacles)):
        index = bisect.bisect_right(longest_increasing_subsequence, obstacles[i])
        if index == len(longest_increasing_subsequence):
            longest_increasing_subsequence.append(obstacles[i])
        else:
            longest_increasing_subsequence[index] = obstacles[i]
        res.append(index + 1)
    return res
