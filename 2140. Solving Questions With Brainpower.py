...
question 0) and make a decision whether to solve or skip each question. Solving question i will earn you pointsi points
but you will be unable to solve each of the next brainpoweri questions. If you skip question i, you get to make the
...
def mostPoints(questions: list[list[int]]) -> int:
    ...
    for i in range(len(questions) - 2, -1, -1):
        ans[i] = questions[i][0]
        skipped = questions[i][1]
        if i + skipped + 1 < len(questions):
            ans[i] += ans[i + skipped + 1]
